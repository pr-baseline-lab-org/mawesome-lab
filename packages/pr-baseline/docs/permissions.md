...
| --------------- | ---------- | -------------------- | --------------- | ---------------------------------------------------------------------------- |
| `check`         | read       | write                | read            | Reads tags and compares commits; writes one status; reads the PR for `--pr`. |
| `sweep`         | read       | write                | read            | Lists open PRs through GraphQL, compares commits, writes statuses.           |
| `move-baseline` | **write**  | write with `--sweep` | read            | Creates or fast-forwards the tag; lists merged PRs by label.                 |
...
| Token            | Creator               | Notes                                                                                                                                                                                                                                                                                                                   |
| ---------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GITHUB_TOKEN`   | `github-actions[bot]` | The action proves the token is the workflow's own and resolves the creator without a request. From the CLI, pass `--creator 'github-actions[bot]'`, because `GET /user` is not available to an installation token. Events it creates trigger no other workflows, so the sweep must run in the same job as the tag move. |
...
2. Set the ruleset's **status source** to match the token: "GitHub Actions" for `GITHUB_TOKEN`, the App for an App token, any source for a PAT. The sweep rewrites a status written by another creator, so the configured token always satisfies the pinned source.
...
Follow the [runbook](./runbook.md) for the order of operations: seed, let the sweep converge, verify with `report`, then require the context.
