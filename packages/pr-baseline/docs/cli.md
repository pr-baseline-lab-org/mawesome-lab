...
| `--ancestry auto\|git\|api`           |                      | `auto`                                      | Ancestry source. `auto` uses git when `--git-dir` (or the working directory) is a full-history clone whose `origin` serves the repository over https or a local path, otherwise the API. `git` fails without such a clone.                                                                                                          |
...
| `--offline`                           |                      | off                                         | `refresh-pr-status` only, and local only: no token, no `--pr`, no status write, no fetch; needs git 2.45 or newer; the commit must resolve in the clone, the base is `refs/remotes/origin/<base>`, and a tag absent from the clone counts as absent while a tag that is not a commit is an error. Rejected for every other command. |
...
`--refresh-pr-statuses` runs a refresh afterwards, also when nothing moved, so a re-dispatch is a safe retry. In a dry run the refresh is evaluated against the intended, unwritten tag positions.
