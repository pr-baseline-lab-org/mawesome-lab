...
describe('sweep', () => {
	...
	it('is idempotent on the second run', async () => {
		const { client, github } = harness({}, populate);
		await client.sweep();
		const again = await client.sweep();
	...
	it('excludes drafts only when configured', async () => {
		...
		expect((await client.sweep()).openPulls).toBe(0);
	});
	...
	it('stops at the per-run write cap and marks the sweep incomplete', async () => {
	...
	it('returns an incomplete summary when the listing is rate limited', async () => {
		...
		const summary = await client.sweep();
	...
	it('refuses to sweep when a baseline is not on the base branch', async () => {
		const { client, github } = harness({}, (gh) => {
		...
		await expect(client.sweep()).rejects.toThrow(/not an ancestor of main/);
		expect(github.requests(/\/statuses\//, 'POST')).toHaveLength(0);
	});
	...
	it('fails before evaluating anything when the creator cannot be resolved', async () => {
		...
		await expect(client.sweep()).rejects.toThrow(/--creator/);
...
describe('sweep guards', () => {
	it('fails outright on the first creator mismatch instead of writing every PR', async () => {
		...
		await expect(client.sweep()).rejects.toThrow(/expected\[bot\]/);
		expect(github.requests(/\/statuses\//, 'POST')).toHaveLength(1);
	});
...
describe('sweep with a custom reporter', () => {
...
describe('sweep write accounting', () => {
...
describe('sweep transport retries', () => {
